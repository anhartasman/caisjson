...
function create_database_proses($table, $table_name)
{
    ...
    switch ($table->process_name)
    {
        case "insert":
            ...
            $content .= '$result_for_' . $table->outputVariable . ' = $obj_entity_factory_tabel_' . $table_name . '->insert_data($data'.$table->outputVariable.');' . "\n";

        break;
        case "update":

        $content .= '$dataupdate' . $table->table_name . '=array();'. "\n";
            ...
            foreach ($table->array_data as $a)
            {
                ...
                $content .= '$dataupdate' . $table->table_name . '[' . get_db_pointer($a->index) . ']=' . create_variable_web($a->value) . ';' . "\n";
            }
            $content .= '$wheredataupdate' . $table->table_name . '=array();'. "\n";
            if (isset($table->where))
            {
                ...
                for ($s = 0;$s < count($table->where);$s++)
                {
                    ...
                    $content .= '$wheredataupdate' . $table_name. '[]=array("column"=>"' . $table->where[$s]->index . '","operation"=>"' . $table->where[$s]->operator . '","value"=>' . create_variable_web($table->where[$s]->value) . ');' . "\n";

                }
            }

            //$content .= '$result_for_' . $table->outputVariable . ' = DB::table(\'' . $table_name . '\')' . "\n";
            $content .= '$result_for_' . $table->outputVariable . ' = $obj_entity_factory_tabel_' . $table_name . '->update_data($dataupdate'.$table->table_name.',$wheredataupdate'.$table_name.');' . "\n";
        ...
        $content .= '$wherearray' . $table_name . '=array();' . "\n";

        for ($s = 0;$s < count($table->where);$s++)
        {
          $content .= '$wherearray' . $table_name. '[]=array("column"=>"'.$table->where[$s]->index.'","operation"=>"'.$table->where[$s]->operator.'","value"=>'.create_variable_web($table->where[$s]->value).');' . "\n";
        //  $content .= '-> where (' . get_db_pointer($table->where[$s]->index) . ', \'' . $table->where[$s]->operator . '\',' . create_variable_web($table->where[$s]->value) . ')' . "\n";

        }
        //$content .= '$result_for_' . $table->outputVariable . ' = DB::table(\'' . $table_name . '\')' . "\n";
        //$content .= '$result_for_' . $table->outputVariable . ' = $obj_entity_factory_tabel_' . $table_name . '->getArrayArray($datahasil_get_hotel_images,$wherearraytb_images_hotel);' . "\n";
        $content .= '$result_for_' . $table->outputVariable . ' = $obj_entity_factory_tabel_' . $table_name . '->delete_data($wherearray' . $table_name.');' . "\n";
        ...
        case "bridge":
            ...
            $content .= '$data' . $table->outputVariable . '[]="id";'. "\n";
            $content .= '$data' . $table->outputVariable . '[]="' . $table->left_bridge_column . '";'. "\n";
            $content .= '$data' . $table->outputVariable . '[]="' . $table->right_bridge_column . '";'. "\n";

            $content .= '$wherearray' . $table_name . '=array();' . "\n";

                $content .= '$wherearray' . $table_name . '[]=array("column"=>"'.$table->left_bridge_column.'","operation"=>"=","value"=>'.create_variable_web($table->left_id).');' . "\n";


            $content .= '$result_for_' . $table->outputVariable . ' = $obj_entity_factory_tabel_' . $table_name . '->getArrayArray($data'.$table->outputVariable.',$wherearray'.$table_name.');' . "\n";
/**
            $content .= '$result_for_' . $table->outputVariable . ' = DB::table(\'' . $table_name . '\')' . "\n";
...
**/
    ...
    switch ($table->process_name)
    {
        ...
        case "select":
            ...
            if (isset($table->where))
            {
              $wherearraytb_images_hotel=array();
              $content .= '$wherearray' . $table_name . '=array();' . "\n";
            ...
            else if ($table->execute == "one")
            {
                ...
                $content .= '$result_for_' . $table->outputVariable . ' = $obj_entity_factory_tabel_' . $table_name . '->getSingleArray($data'.$table->outputVariable.',$wherearray'.$table_name.');' . "\n";
        ...
        {
          if ($table->process_name == "insert" || $table->process_name == "update" || $table->process_name == "delete")
          {
            $content .= '$' . $table->outputVariable . ' = $result_for_' . $table->outputVariable . '["body"];' . "\n\n";
          }else{
            ...
            if (isset($table->process_name))
            {
                ...
                {
                    $content .= '$' . $table->outputVariable . '_last_id = $' . $table->outputVariable.'["last_id"];'. "\n";
        ...
        if ($table->process_name == "bridge")
        {
            ...
            $content .= 'if(!in_array($' . $table->outputVariable . '[$ib]["' . $table->right_bridge_column . '"],' . create_variable_web($table->right_array) . ')){' . "\n";
              ...
              $content .= '$wherearray' . $table_name . '=array();' . "\n";
                ...
                $content .= '$wherearray' . $table_name. '[]=array("column"=>"id","operation"=>"=","value"=>$'.$table->outputVariable.'[$ib]["id"]);' . "\n";


              $content .= '$obj_entity_factory_tabel_' . $table_name . '->delete_data($wherearray' . $table_name.');' . "\n";
            ...
            $content .= 'for($ir=0; $ir<$bedajum_bridge_' . $table->outputVariable . '; $ir++) {' . "\n";
              $content .= '$datainsert' . $table->table_name . '=array();'. "\n";
              $content .= '$datainsert' . $table->table_name . '["'.$table->right_bridge_column.'"]=' . create_variable_web($table->right_array) . '[$ir];'. "\n";
              $content .= '$datainsert' . $table->table_name . '["'.$table->left_bridge_column.'"]=' . create_variable_web($table->left_id) . ';'. "\n";
              $content .= '$hasilinserttabel' . $table->table_name . ' = $obj_entity_factory_tabel_' . $table_name . '->insert_data($datainsert'.$table->table_name.');' . "\n";
              $content .= "\n";
...
/**
            ...
            **/
            ...
            $content .= '$dataupdate' . $table->table_name . '=array();'. "\n";
            $content .= '$dataupdate' . $table->table_name . '["' . $table->right_bridge_column . '"]=' . create_variable_web($table->right_array) . '[$bedajum_bridge_'. $table->outputVariable . '+$ir];'. "\n";
            $content .= '$wheredataupdate' . $table->table_name . '=array();'. "\n";
            $content .= '$wheredataupdate' . $table_name. '[]=array("column"=>"id","operation"=>"=","value"=>$result_for_'.$table->outputVariable.'[$ir]["id"]);' . "\n";

            $content .= '$obj_entity_factory_tabel_' . $table_name . '->update_data($dataupdate'.$table->table_name.',$wheredataupdate'.$table_name.');' . "\n";
...
**/
            ...
            $content .= '$dataupdate' . $table->table_name . '=array();'. "\n";
            $content .= '$dataupdate' . $table->table_name . '["' . $table->right_bridge_column . '"]=' . create_variable_web($table->right_array) . '[$ir];'. "\n";
            $content .= '$wheredataupdate' . $table->table_name . '=array();'. "\n";
            $content .= '$wheredataupdate' . $table_name. '[]=array("column"=>"id","operation"=>"=","value"=>$result_for_'.$table->outputVariable.'[$ir]["id"]);' . "\n";
            ...
            elseif (is_object($category))
            {
                ...
                switch ($category->scannedkey[0])
                {
                    ...
                    case "daf_api":
                        ...
                        $hasilcekneed = what_need_from_daf_api($category, $theconfig);
                        $dapatarwork = $hasilcekneed->ar_worktodo;

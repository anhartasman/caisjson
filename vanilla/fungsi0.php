...
function create_booleancheck_web(stdClass $checkgroup){
  ...
  if($checkgroup->content[$c]->type=="group"){
    $content.="(".create_booleancheck_web($checkgroup->content[$c]).")";
    ...
    if(!isset($checkgroup->content[$c]->check->var_type)){
      ...
      $content.=create_variable_web($checkgroup->content[$c]->check).' '.$checkgroup->content[$c]->operator.' '.create_variable_web($checkgroup->content[$c]->value)."\n";
                    ...
                    if($thepage->engine[$eng]->content[$ca]->id==$pro->id){
                      ...
                      $getengine=render_engine("session",$f,$pro,null);
                       ...
                       break;
                              ...
                              if(isset($pro->direct) && $pro->direct==true){
                              $getengine=render_engine("session",$pro,$pro,null);
                              $content.=$getengine->content."\n";
                              }
            break;
            case "change_page":
              $dapetmesin=1;
...
function render_engine($type,$engine,$pro,$action){
  ...
  switch($type){
    case "url_catcher":
          ...
          $thecatcher=$engine;
         $content.='$'.$engine->variable.' = null;'."\n";
         $content.='for($i=0; $i<count($url_catch); $i++){'."\n";
         $content.='if($url_catch[$i]=="'.$thecatcher->catch.'"){'."\n";
         $content.='if($i+1<=count($url_catch)){'."\n";
         $content.='$'.$thecatcher->variable.' = $url_catch[$i+1];'."\n";
         $content.='$variables[\''.$thecatcher->variable.'\'] = $'.$thecatcher->variable.';'."\n";
         $content.='}'."\n";
         $content.='break;'."\n";
         $content.='}'."\n";
         $content.='}'."\n";
    ...
    $content.=$objproses->content;
              ...
              if($table_action!=null){
               ...
               if(count($pro->runifnotnull)>0){
                 ...
                 for($pa=0; $pa<count($pro->runifnotnull); $pa++){
                   if(!isset($pro->runifnotnull[$pa]->var_type)){
                      $table_action->$pro[$pa]->var_type="variable";
                    }
                   $content.=' '.create_variable_web($pro->runifnotnull[$pa]).'!=null';
                   if($pa+1<count($pro->runifnotnull)){
                     $content.=' &&';
                   }
                 }
...
$content.="if (".create_booleancheck_web($objchecksession)."){"."\n";
    ...
    switch($table->execute){
      ...
      $bahan_output="";
                   ...
                   if($table->engine[$eng]->type=="table"){
                     for($ca=0; $ca<count($table->engine[$eng]->content); $ca++){
                       if($table->engine[$eng]->content[$ca]->id==$pro->id){
                         ...
                         $getengine=render_engine("table",$table->engine[$eng]->content[$ca],$pro,null);
                         //echo $table->engine[$eng]->content[$ca]->process_name;
                         $varphpawal.=$getengine->varphpawal."\n";
                         $content.=$getengine->content."\n";
        ...
        for($l=1;$l<count($ledakan);$l++) {
                ...
                $dapatkurunganlama="{".$ledakandalam[0]."}";
          ...
          //$content.='$bahan_output'.$table->outputVariable.'=str_replace("'.$dapatkurunganlama.'",'.$bikinvar.',$bahan_output'.$table->outputVariable.');'."\n";

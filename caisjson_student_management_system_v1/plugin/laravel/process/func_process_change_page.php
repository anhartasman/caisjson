<?php
function func_process_change_page($engine,$pro,$action){
  ...
  //------------------------
  //tempat skrip
  if(!isset($engine->param)){
    $engine->param=array();
  }
$thehref=get_project_url_php($engine->modul,$engine->page,$engine->param);
  $content.='header("Location: '.$thehref.'");'."\n";
  $content.='die();';
  //------------------
  //balikan
